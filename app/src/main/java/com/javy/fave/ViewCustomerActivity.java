...
public class ViewCustomerActivity extends AppCompatActivity implements OnSuccessListener {
    private ActionBar actionBar;
    private Toolbar toolbar;
    private Button endCustomerButton;
...
//    private LocationCallback locationCallback;
    ...
    private EditText customerDetailsEditText;
...
//        dbHelper.createTable();
        ...
        NetworkUtils.
                fetchData("GET",endpoint,
                        null, ViewCustomerActivity.this,
                        ViewCustomerActivity.this);
    ...
    public void onSuccess(ResponseBody responseBody) {
        ...
        try {
            ...
            if (result != null && result.contains("customer") &&
                    endpoint.equals("customer/"+customerId)){

                JSONObject jsonObject = new JSONObject(result);
                if (jsonObject.has("customer") &&
                        jsonObject.get("customer") instanceof JSONObject
                ) {

                    JSONObject customer = jsonObject.getJSONObject("customer");


                    statusEditText.setText(customer.
                            getString("email"));
                    amountEditText.setText(customer.
                            getString("phone"));
                    customerIdEditText.setText(customer.getString("name"));
//                    dateEditText.setText(
//                            customer.getString("date"));
...
//                    if ( customer.getString("status").equals("start") ) {
//
//                        if (customer.has("customer") &&
//                                customer.get("customer") instanceof JSONObject
//                        ) {
//                            contactCustomerButton.setVisibility(View.VISIBLE);
//                            customerLayout.setVisibility(View.VISIBLE);
//                            customerNameEditText.setText(customer.getJSONObject("customer").getString("name"));
//                            customerPhoneEditText.setText(customer.getJSONObject("customer").getString("phone"));
//
//                        }
////                        else{
...
//                        endCustomerButton.setVisibility(View.VISIBLE);
...
//                    Toasts.toastIconError(IndividualDealActivity.this,"Error Loading Account. Please Try Again");
                    Toast.makeText(getBaseContext(), "Error Loading Customer. Please Try Again", Toast.LENGTH_SHORT).show();
//                    FirebaseCrashlytics.getInstance().recordException(new Exception("Add Account Error: "+result));
                    ...
                    endpoint.equals("endcustomer/"+customerId))
            {
                JSONObject jsonObject = new JSONObject(result);
...
//                    mCustomerId = mPrefs.getString("customerId", null);
                    ...
                    mEditor.putString("customerId", null).commit();

//                    Toasts.toastIconSuccess(EditMeetingActivity.this,message);
                    Toast.makeText(ViewCustomerActivity.this, message,
...
//                            "Error Editing Meeting. Please Try Again");
                    Toast.makeText(ViewCustomerActivity.this,
                            "Error Ending Customer. Please Try Again",Toast.LENGTH_SHORT).show();
//                    FirebaseCrashlytics.getInstance().recordException(new Exception("Error Ending Customer Error: "+result));
    ...
    public boolean onOptionsItemSelected(MenuItem item) {
        switch (item.getItemId()) {
            case android.R.id.home:
                onBackPressed();
                Intent intent = new Intent(ViewCustomerActivity.this, CustomersActivity.class);
