...
//    private LocationCallback locationCallback;
    ...
    private EditText orderDetailsEditText;
...
//        dbHelper.createTable();
        ...
        orderDetailsEditText = (EditText) findViewById(R.id.orderDetailsEditText);
...
//        });
    ...
    public void onSuccess(ResponseBody responseBody) {
        ...
        try {
            ...
            if (result != null && result.contains("order") &&
                    endpoint.equals("order/"+orderId)){

                JSONObject jsonObject = new JSONObject(result);
                if (jsonObject.has("order") &&
                        jsonObject.get("order") instanceof JSONObject
                ) {

                    JSONObject order = jsonObject.getJSONObject("order");


                    orderDetailsEditText.setText(order.
                            getString("order_details"));
                    amountEditText.setText(order.
                            getString("value"));
                    customerNameEditText.setText(
                            order.getJSONObject("customer")
                                    .getString("name"));
...
//                    if ( order.getString("status").equals("start") ) {
//
//                        if (order.has("customer") &&
//                                order.get("customer") instanceof JSONObject
//                        ) {
//                            contactCustomerButton.setVisibility(View.VISIBLE);
//                            customerLayout.setVisibility(View.VISIBLE);
//                            customerNameEditText.setText(order.getJSONObject("customer").getString("name"));
//                            customerPhoneEditText.setText(order.getJSONObject("customer").getString("phone"));
//
//                        }
////                        else{
...
//                        endOrderButton.setVisibility(View.VISIBLE);
...
//                    Toasts.toastIconError(IndividualDealActivity.this,"Error Loading Account. Please Try Again");
                    Toast.makeText(getBaseContext(), "Error Loading Order. Please Try Again", Toast.LENGTH_SHORT).show();
//                    FirebaseCrashlytics.getInstance().recordException(new Exception("Add Account Error: "+result));
                    ...
                    endpoint.equals("endorder/"+orderId))
            {
                JSONObject jsonObject = new JSONObject(result);
...
//                    mOrderId = mPrefs.getString("orderId", null);



                    mEditor.putString("orderId", null).commit();
...
//                            "Error Editing Meeting. Please Try Again");
                    Toast.makeText(ViewOrderActivity.this,
                            "Error Ending Order. Please Try Again",Toast.LENGTH_SHORT).show();
//                    FirebaseCrashlytics.getInstance().recordException(new Exception("Error Ending Order Error: "+result));
    ...
    public boolean onOptionsItemSelected(MenuItem item) {
        switch (item.getItemId()) {
            case android.R.id.home:
                onBackPressed();
                Intent intent = new Intent(ViewOrderActivity.this, OrdersActivity.class);
